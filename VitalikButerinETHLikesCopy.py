...
aantonopTweets = pd.read_csv('VitalikTweetsInfo.csv', usecols = colsList)
aantonopTweets = aantonopTweets.reset_index(drop=True)
colsListBTC = ["Date","Close"]
allTimeBitcoin = pd.read_csv('coin_Ethereum.csv', usecols = colsListBTC)
...
aantonop2021BTCLine = px.line(data_frame = allTimeBitcoin, title = 'Ethereum Value by Date vs Vitalik Buterin likes', x = 'Date', y = 'Close')
for i in range(0,len(tbd['date'])):
    ...
    try:
        ...
        if (tbd.at[i,'sentiment'] == 'neutral'):
            aantonop2021BTCLine.add_annotation(x = tbd.at[i,'date'], y = yv,text = f'{tbd.at[i,"tweets"]} tweets {tbd.at[i,"likes"]} likes {tbd.at[i,"probability"]} average probability', showarrow = True, font=dict(family="sans serif",size=12,color="black"))
        elif (tbd.at[i,'sentiment'] == 'positive'):
            aantonop2021BTCLine.add_annotation(x = tbd.at[i,'date'], y = yv,text = f'{tbd.at[i,"tweets"]} tweets {tbd.at[i,"likes"]} likes {tbd.at[i,"probability"]:.3f} average probability', showarrow = True, font=dict(family="sans serif",size=12,color="green"))
            ...
            aantonop2021BTCLine.add_annotation(x = tbd.at[i,'date'], y = yv,text = f'{tbd.at[i,"tweets"]} tweets {tbd.at[i,"likes"]} likes {tbd.at[i,"probability"]:.3f} average probability', showarrow = True, font=dict(family="sans serif",size=12,color="red"))
        ...
        x=0
aantonop2021BTCLine.add_annotation(text='Sentiment Analysis:<br>Red = Negative Tweet <br>Green = Positive Tweet<br>Black = Neutral Tweet<br>Probability: Odds the analysis is correct', 
                    align='left',
