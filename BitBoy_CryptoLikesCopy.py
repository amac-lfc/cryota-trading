...
aantonopTweets = pd.read_csv('BitBoy_CryptoInfo.csv', usecols = colsList)
...
for i in range(2,10):
    ...
    j = i - 1
    if(j<9):
        allTimeBitcoin = allTimeBitcoin[allTimeBitcoin['Date'].str.contains(f'2020-0{j}') == False]
...
aantonopTweets = aantonopTweets[aantonopTweets['text'].str.contains('RT') == False]
...
aantonop2021BTCLine = px.line(data_frame = allTimeBitcoin, title = 'Bitcoin Value by Date vs BitBoy_Crypto likes', x = 'Date', y = 'Close')
...
aantonop2021BTCLine.add_annotation(text='Sentiment Analysis:<br>Red = Negative Tweet <br>Green = Positive Tweet<br>Black = Neutral Tweet', 
                    ...
                    bordercolor='black',
                    borderwidth=2)
BitBoy_Crypto2021BTCLine = aantonop2021BTCLine
# BitBoy_Crypto.show()